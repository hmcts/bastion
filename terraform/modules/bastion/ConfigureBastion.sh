...
then
  ...
  do
      ...
      then
        ...
        logger -s  "custom_script:: creating the FS on $id1"
          mkfs.xfs /dev/$id1
        partprobe /dev/$id1
        if [[ $size =~ \d*G ]]
        then
          logger -s "custom_script:: mounting /dev/$id1 to /opt/data-migration"
          if [[ !  -d /opt/data-migration ]]
            then
              mkdir /opt/data-migration
          fi
          mount /dev/$id1 /opt/data-migration
          uuid=$(blkid -s UUID -o value /dev/$id1)
